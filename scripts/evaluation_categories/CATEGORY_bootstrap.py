...
COL_SCORE   = "match"    # per-sample binary (0 or 1)
...
diff_best_df = pd.DataFrame(
    rows_best,
    columns=["prompt", "Δsuccess_vs_best",
...
diff_base_df = pd.DataFrame(
    rows_base,
    columns=["prompt", "Δsuccess_vs_baseline",
             f"{int((1-ALPHA)*100)}%_CI_low",
             f"{int((1-ALPHA)*100)}%_CI_high",
             "approx_p_value"],
).set_index("prompt")
...
# 8  Write CSVs
# ---------------------------------------------------------------------
ci_df.to_csv(OUTPUT_DIR / "success_bootstrap_results_per_prompt.csv", index=False)
print("→ bootstrap_results_per_prompt.csv written")

diff_best_df.to_csv(OUTPUT_DIR / "success_bootstrap_pairwise_vs_best.csv")
print(f"→ bootstrap_pairwise_vs_best.csv written (best = {best_prompt})")

diff_base_df.to_csv(OUTPUT_DIR / "success_bootstrap_pairwise_vs_baseline.csv")
